...
- [ ] **Deprecation lifecycle, un-retiring** — a concept retired in error, put back
      > The write half of the lifecycle, and the one operation `adr/0040` and `adr/0041` both defer.
      > It goes through the candidate seam like every other write. It is a **retraction** —
      > `owl:deprecated` and any `dcterms:isReplacedBy` come out — which makes it the first
      > operation in this build whose whole purpose is to remove statements, and the first that
      > must decide what happens to the `skos:changeNote` explaining a retirement that is now
      > undone.
